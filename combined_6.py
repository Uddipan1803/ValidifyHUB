...
def register():
    if request.method == 'POST':
        ...
        # Check if the username is already taken (you may want to add more checks)
        try:
...
def add_user():
    if request.method == 'POST':
        ...
        try:
            ...
            # Insert the user's name and email into the 'email_sample_1' table
...
def user_page(sl_no):
    # Your logic to handle the user-specific page here
    # You can use the user_email as needed
    return render_template('upload2_2.html', sl_no=sl_no)

@app.route('/upload_aadhar', methods=['POST'])
def upload_aadhar_file():
    pdf_file = request.files.get('pdf_file')  # Use request.files.get to avoid KeyError
    sl_no = session.get('sl_no')

    if pdf_file:
        status_message = common_module.upload_and_process(pdf_file, common_module.process_aadhar, common_module.send_to_database_aadhar, sl_no)
...
def upload_pan_file():
    ...
    if pdf_file:
        status_message = common_module.upload_and_process(pdf_file, common_module.process_pan, common_module.send_to_database_pan, sl_no)
        if status_message == "Successfully Uploaded":
...
def update_status():
    if session.get('aadhar_submitted') and session.get('pan_submitted'):
        try:
            # Connect to the MySQL database
            conn = mysql.connector.connect(**db_config)
            cursor = conn.cursor()
